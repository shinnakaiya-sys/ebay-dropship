...
jan_research2.py
================
JANコードまたはASINから4ステップでリサーチする。
（jan_research.py のASIN対応版 — JANとASINを混在して渡せる）

識別子の自動判定:
  13桁数字           → JANコード（従来通り）
  10桁英数字(B0...)  → ASIN → Keepaで商品情報＋eanListからJANを復元
                        JAN復元できればJANでTerapeak/eBay検索、
                        できなければ型番・タイトルでフォールバック検索

Step 1: Terapeak 販売実績確認（過去30日）
Step 2: Keepa 仕入れ価格・重量取得（JANは code=、ASINは asin= で取得）
Step 3: eBay Active Listings 最安値・URL取得（新品のみ）
Step 4: 利益計算 → GO/No-Go判定

使い方:
  python3 jan_research2.py --account kozuki 4901777321991
  python3 jan_research2.py --account kaworu 4901777321991 B0ABC12345
  python3 jan_research2.py --account dbz --dry-run B0ABC12345
"""
...
def is_asin(code: str) -> bool:
    """10桁英数字かつ純数字でないものをASINとみなす（B0...等）"""
    return bool(ASIN_RE.match(code)) and not code.isdigit()
...
def extract_model_from_title(title: str) -> list[str]:
    """タイトルから型番らしき文字列を抽出（Terapeak/eBayフォールバック検索用）"""
    patterns = [
...
# ==========================================
def get_terapeak_sold_count(keyword: str, _unused: str, driver) -> tuple[int, str]:
    """
    Terapeak（Seller Hub Research）で過去30日の販売数と代表タイトルを返す。
    JANまたは型番などのキーワードで検索し、複数行ヒット時は総販売数を合計。
    """
    print(f"           Teapeakキーワード: {keyword}")

    try:
        do_research(driver, keywords=keyword, days=30, min_price=0, category_id="")
...
# ==========================================
# Step 2: Keepa で仕入れ情報取得（JAN / ASIN 両対応）
# ==========================================
def get_keepa_info(code: str, api_key: str, by_asin: bool = False
                   ) -> tuple[str | None, str | None, str | None, float | None, str]:
    """
    Keepa API でJANコードまたはASINから商品情報を取得。
    戻り値: (商品名, 価格文字列, Amazon URL, 請求重量kg, 復元JAN)
      - 復元JAN: eanListから取得した13桁JAN（日本製45/49を優先）。なければ ""
    ...
    try:
        ...
        resp = requests.get("https://api.keepa.com/product",
        ...
        # eanListからJANを復元（日本製45/49を優先、なければ最初の13桁）
        recovered_jan = ""
        ean_list = [str(c).strip() for c in (product.get("eanList") or [])]
        for c in ean_list:
            if is_japan_jan(c):
                recovered_jan = c
                break
        if not recovered_jan:
            for c in ean_list:
        ...
        return title, price_str, url, weight_kg, recovered_jan
...
# ==========================================
def get_ebay_active_lowest(jan: str, token: str,
                           fallback_keyword: str = "") -> tuple[float, str]:
    """
    新品かつ最安値のeBay出品価格(USD)と出品URLを返す。
    JANがあればGTIN検索 → キーワード検索の順。
    JANがない場合（ASINでJAN復元不可）はfallback_keyword（型番等）で検索。
    """
    ...
    try:
        ...
        # 見つからない場合はキーワード検索（JAN → 型番の順）
        if not items:
            keyword = jan or fallback_keyword
            if keyword:
                params2 = {
                    "q":      keyword,
                    "filter": "conditions:{NEW}",
...
# スプレッドシート書き込み
# ==========================================
def write_to_sheet(ws, identifier: str, result: dict, dry_run: bool):
    """
    「新品出品下書き」タブに1行追記する。
    identifier: JAN（ASIN入力でJAN復元できた場合は復元JAN、できなければASIN）
    列構成（A〜T）:
      A=JAN/ASIN, B=商品名, C=タイトル(eBay), D=カテゴリ, E=コンディション,
    ...
    if dry_run:
        print(f"  [DRY-RUN] 書き込み予定: {identifier} | 判定={judgment} | "
...
# 1件の識別子（JAN / ASIN）を処理
# ==========================================
def research_one(code: str, token: str, rate: float, ws, dry_run: bool,
                 account: str = "kozuki", force: bool = False,
                 manual_sold: int = 0, driver=None) -> dict:
    by_asin = is_asin(code)
    kind = "ASIN" if by_asin else "JAN"
    ...
    # ── Keepa: 商品名・価格・URL・重量・JAN復元（Step1/2で共用）────
    keepa_key = _get_keepa_api_key()
    print("  [Keepa] 商品情報取得中...")
    amazon_title, amazon_price_str, amazon_url, weight_kg, recovered_jan = (
        get_keepa_info(code, keepa_key, by_asin=by_asin)
        if keepa_key else (None, None, None, None, "")
    )

    # 検索に使うJANを確定（JAN入力ならそのまま / ASIN入力なら復元JAN）
    jan = recovered_jan if by_asin else code
    if by_asin:
        if recovered_jan:
            print(f"          JAN復元: {recovered_jan} ✅")
        else:
            print(f"          JAN復元: 不可 → 型番フォールバックで検索")
    ...
    if not keepa_key:
        ...
        if by_asin:
            print("  → ❌ ASIN処理にはKeepaが必須。スキップ")
            return {"status": "skipped", "reason": "KEEPA_API_KEY未設定"}

    # ASINでKeepaに商品が見つからない場合はスキップ
    if by_asin and not amazon_title and not amazon_price_str:
        print("  → ❌ KeepaでASINが見つかりません。スキップ")
        return {"status": "skipped", "reason": "Keepa商品未発見"}

    # フォールバック検索キーワード（JANなし時のTerapeak/eBay用）
    fallback_kw = ""
    if not jan and amazon_title:
        models = extract_model_from_title(amazon_title)
        fallback_kw = models[0] if models else amazon_title[:40]
    ...
    if force:
        ...
        search_kw = jan or fallback_kw
        if not search_kw:
            print("  → ❌ 検索キーワードなし（JAN・型番とも不明）。スキップ")
            return {"status": "skipped", "reason": "検索キーワードなし"}
        print(f"  [Step 1] Terapeak販売実績確認（過去30日・{'JAN' if jan else '型番'}検索）...")
        sold_count, ebay_title = get_terapeak_sold_count(search_kw, "", driver)
    ...
    ebay_usd, ebay_url = get_ebay_active_lowest(jan, token, fallback_keyword=fallback_kw)
    ...
    product_name = amazon_title or ebay_title or code
    ...
    # シートA列: JAN復元できた場合は復元JAN、できなければASIN
    write_to_sheet(ws, jan or code, result, dry_run)
    return result
...
def main():
    ...
    if not codes:
        print("使い方: python3 jan_research2.py --account <アカウント名> <JAN/ASIN>...")
        print("例:     python3 jan_research2.py --account kozuki 4901777321991")
        print("        python3 jan_research2.py --account kaworu 4901777321991 B0ABC12345")
        sys.exit(1)

    mode = "[DRY-RUN]" if dry_run else "[本番]"
    print(f"\n{'='*55}")
    print(f"  JAN/ASINリサーチ {mode}")
    ...
    try:
        for code in codes:
            code = code.strip()
            # ASINは大文字に正規化
            if re.fullmatch(r"[A-Za-z0-9]{10}", code) and not code.isdigit():
                code = code.upper()

            # バリデーション: 10桁以上の数字（JAN）または ASIN
            if not ((code.isdigit() and len(code) >= 10) or is_asin(code)):
                print(f"\n  ⚠️  無効な識別子: {code}（スキップ）")
                continue

            result = research_one(code, token, rate, ws, dry_run, account=account,
                                  force=force, manual_sold=manual_sold, driver=t_driver)

            if result["status"] == "skipped":
                summary["skipped"].append(code)
            elif result.get("is_go"):
                summary["go"].append(code)
            else:
                summary["no_go"].append(code)
